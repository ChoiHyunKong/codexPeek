...
| 작은 화면 대응 | 기본 크기에서는 한도명·잔여 비율·막대·리셋 시간, 높이 150 미만에서는 막대만 표시 |
...
기본 크기에서는 `주간 한도`, `일간 한도`, `5시간 한도` 등 실제 한도명이 표시됩니다. 높이를 150 미만으로 줄이면 한도명·비율·리셋 시간·일반 갱신 안내를 숨기고 그래프 바를 유지합니다. 작은 화면에서도 바에 마우스를 올리면 한도명, 잔여 비율, 리셋 시간, 최근 갱신 시각을 확인할 수 있습니다. 조회 중이거나 갱신에 실패하면 상태 안내는 계속 표시합니다.

![높이를 줄인 화면 — 그래프 바 유지](artifacts/minimum-demo.png)
...
| 창 크기 | 270 × 160 | 최소 220 × 130, Windows 배율 적용 |
...
2026-09-23 UI 수정 후에는 기본 크기, 최소 크기, 높이 149/150의 전환 경계, 크기 복원, 확장 크기를 렌더링했습니다. 두 한도의 바가 유지되고 스크롤 없이 표시되는지, 축소 시 글자가 숨겨지고 확대 시 복원되는지 추가 확인했습니다.
